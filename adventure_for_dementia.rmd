...
daylight_hours <- getSunlightTimes(x,lat = -43.5, lon = 172.6, tz = "Pacific/Auckland",
  ...
  mutate(expected_time_1.25 = case_when(is.na(duration) ~ 
                                          as.duration(month * max(mean_lap_time,na.rm = T)),
                                        TRUE ~ duration),
         expected_time_1.50 = case_when(is.na(duration) ~ as.duration(month*3600 * 1.5),
                                        TRUE ~ duration),
  ...
  pivot_longer(names_to = "lap_time",
               ...
               cols= contains("expected_time")) %>% 
  mutate(lap_time = factor(case_when(done ~ "actual",
                                     lap_time %in% "expected_time_1.25" ~ "mean",
                                     TRUE ~ "max"),
...
ggsave(filename = "outputs/running_in_the_dark.png",width = 15, height = 15,units="cm")

daylight_hours %>% 
  filter(date %in% as.Date("2023-07-01")) %>% 
  mutate()
```
