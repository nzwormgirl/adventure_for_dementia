...
pop_repeats <- map_dfr(1:12,function(x) make_monthly_repeats(pop,x)) %>% 
  ...
  mutate(done = case_when(Month %in% c("January","February","March","April","May") ~ TRUE,
                          TRUE ~ FALSE)) %>% glimpse
...
animated_plot <- monthly_efforts %>% 
  ...
  scale_colour_paletteer_d(palette = "rcartocolor::Vivid") #Bold
...
anim_save(animated_plot_gif,filename = "outputs/animated_plot.gif")
...
anim_save(animated_plot_mp4,filename = "outputs/animated_plot.mp4")
