...
walk(month.name, function(x){
 ...
 ggsave(file = paste0("outputs/elevation_",x,".png"),dpi=72,width = 20, height = 20, units = "cm")
})
...
afd_january <- read_gpx("data/Adventure_for_Dementia_January.gpx") %>% 
  mutate(month = 1,
         done = TRUE,
         emoji = unique(month_labels$emoji)[1],
         date_time = parse_date_time(time,tz = "UTC",orders = "%Y-%m-%d%H:%M%S" ) %>% 
           with_tz("Pacific/Auckland"),
         diff_time = as.numeric(date_time - min(date_time,na.rm=T))
         ) %>%
  calculate_distance %>% 
  na.omit() %>% glimpse

plot_january <- afd_january %>% 
  plot_run() +
  scale_colour_manual(values = c("black","black")) +
  geom_point(show.legend = FALSE) +
  theme(plot.title = element_text(colour = "transparent")) +
  transition_reveal(diff_time) +
  labs(subtitle = 'Time: {round(frame_along/60,0)} minutes',
       caption = NULL)
  

animate_january <- gganimate::animate(plot_january,res=72,width = 20, height = 20, units = "cm",fps=10,start_pause = 5, end_pause = 5)

anim_save(animate_january,filename = "outputs/january_animation.gif")
...
tile_z11 <- get_tiles(st_bbox(st_buffer(st_transform(pop_track,3857),350)),
                       provider = "Esri.WorldTopoMap",#OpenTopoMap",#Stamen.Terrain",
                      crop = TRUE, zoom = 15)
...
image_write(new_gif, "january_combined_animation.gif")
```
